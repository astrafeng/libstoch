...
#include "libstoch/branching/solvePDEDYMC.h"
...
int main(int argc, char *argv[])
{
    ...
    pair<double, double > anal = libstoch::estimateHJB(nbSimAnal, 1., point, mat, g);
    if (world.rank() == 0)
    ...
    for (int isw = 0; isw < nbSwitch.size(); ++isw)
    {
        ...
        for (int ipart = 0; ipart < nbpart; ++ipart)
        {
            ...
            std::tuple< double, Eigen::Array<double, 6, 1>, double,  Eigen::Array<double, 6, 1> >    val = libstoch::solvePDEDYMCEuler<6, libstoch::ExpDist, trng::yarn5>(mu, vol, fNonLin, point, 0., mat, law,  g, nbSim, stepEuler, gen);
