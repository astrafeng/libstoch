...
#include "libstoch/branching/solvePDEDY2MC.h"
...
int main(int argc, char *argv[])
{
    ...
    double solRef = libstoch::estimatePortfolio<4>(muSj1, muVolSj1, revertSj1, sigVolSj1, sigS1, mat, eta, x1, x0, 1000000, 200);
    ...
    for (int isw = 0; isw < nbSwitch.size(); ++isw)
    {
        ...
        for (int ipart = 0; ipart < nbpart; ++ipart)
        {
            ...
            std::tuple< double, Eigen::Array<double, 5, 1>, Eigen::Array<double, 15, 1>, double,  Eigen::Array<double, 5, 1>, Eigen::Array<double, 15, 1>   >   val = libstoch::solvePDEDY2MCEuler<5, libstoch::ExpDist, trng::yarn4>(mu, vol, fNonLin, point, 0., mat, law,  g, nbSim, stepEuler, gen);
