...
double  testCase1LinearInterpolation(const int &p_ndt,  const  shared_ptr<libstoch::SpaceGrid>   &p_grid)
{
#ifdef USE_MPI
...
BOOST_AUTO_TEST_CASE(TestSemiLagrang1Lin)
{
    ...
    shared_ptr<libstoch::SpaceGrid>  grid = make_shared<RegularSpaceGrid>(lowValues, step, nstep);
...
BOOST_AUTO_TEST_CASE(TestSemiLagrang1Quad)
{
    ...
    ArrayXd step  = ArrayXd::Constant(2, 4 * M_PI / nmesh);
    ArrayXi nstep = ArrayXi::Constant(2, nmesh);
    shared_ptr<libstoch::SpaceGrid>  grid = make_shared<RegularLegendreGrid>(lowValues, step, nstep, npoly);
    double error = testCase1LinearInterpolation(ndt,  grid);
...
BOOST_AUTO_TEST_CASE(TestSemiLagrang1SparseQuad)
{
    ...
    shared_ptr<libstoch::SpaceGrid>  grid = make_shared<SparseSpaceGridBound>(lowValues, sizeDomain, levelMax, weight, degree);
    double error = testCase1LinearInterpolation(ndt,  grid);
    BOOST_CHECK(error < 0.006);
}
...
BOOST_AUTO_TEST_CASE(TestSemiLagrang1SparseQuadAdapt)
{
    ...
    shared_ptr<libstoch::SparseSpaceGridBound>  grid = make_shared<SparseSpaceGridBound>(lowValues, sizeDomain, levelMax, weight, degree);
