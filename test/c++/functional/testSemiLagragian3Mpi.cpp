...
void  testCase3LinearInterpolation(const int &p_ndt,  const  shared_ptr<libstoch::FullGrid>   &p_grid, const double &p_proba, const bool &p_bOneFile)
{
...
BOOST_AUTO_TEST_CASE(TestSemiLagrang3LinOneFile)
{
    ...
    shared_ptr<libstoch::FullGrid>  grid = make_shared<RegularSpaceGrid>(lowValues, step, nstep);
    // one file tu dump
    bool bOneFile = true;
...
BOOST_AUTO_TEST_CASE(TestSemiLagrang3Quad)
{
    ...
    shared_ptr<libstoch::FullGrid>  grid = make_shared<RegularLegendreGrid>(lowValues, step, nstep, npoly);
    // one file tu dump
    bool bOneFile = true;
    testCase3LinearInterpolation(ndt,  grid, proba, bOneFile);
}
