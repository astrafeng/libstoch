...
class OptimizeStorageWithDemandBaseSDDP: public libstoch::OptimizerSDDPBase
{
...
protected :
                          ...
                          Eigen::ArrayXd &p_stateFollowing, double &p_cost) const
    {
        ...
        for (int isto = 0 ; isto < m_nbStorage; ++isto)
        {
            ...
            upperBound(isto + 2 * m_nbStorage) = libstoch::infty;
        }
        // add demand variable
        lowBound(3 * m_nbStorage) = 0.;
        upperBound(3 * m_nbStorage) = libstoch::infty;

        // quantity bought on the spot
        lowBound(3 * m_nbStorage + 1) = 0.;
        upperBound(3 * m_nbStorage + 1) = libstoch::infty;
        // for  fictitious data for bellman
        lowBound(3 * m_nbStorage + 2) = - libstoch::infty;
        upperBound(3 * m_nbStorage + 2) = libstoch::infty;
...
/// \param   p_timeDAverage        average demand
    ...
    OptimizeStorageWithDemandBaseSDDP(const double &p_withdrawalRate,  const int &p_nbStorage,
                                      ...
                                      const std::shared_ptr<libstoch::OneDimData<libstoch::OneDimRegularSpaceGrid, double> >    &p_timeInflowAver,
                                      const  double   &p_sigD, const double &p_kappaD,
                                      const std::shared_ptr<libstoch::OneDimData<libstoch::OneDimRegularSpaceGrid, double> > &p_timeDAverage,
