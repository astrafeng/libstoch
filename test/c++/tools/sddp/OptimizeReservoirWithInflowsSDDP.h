...
#include "libstoch/sddp/SDDPCutOptBase.h"
#include "libstoch/sddp/OptimizerSDDPBase.h"
#include "libstoch/core/grids/OneDimRegularSpaceGrid.h"
#include "libstoch/core/grids/OneDimData.h"
#include "libstoch/core/utils/comparisonUtils.h"
#include "libstoch/core/utils/constant.h"
...
public:
    ...
    void addConstraints(const libstoch::SDDPCutOptBase &p_linCut, int p_nbStorage, Eigen::ArrayXi &p_rows,   Eigen::ArrayXi   &p_columns,  Eigen::ArrayXd   &p_elements,
                        Eigen::ArrayXd    &p_lowBoundConst,  Eigen::ArrayXd   &p_upperBoundConst) const
    {
        ...
        for (int icut = 0 ; icut < cuts.cols() ; ++icut)
        {
            ...
            p_upperBoundConst(ibound + icut) = libstoch::infty;
        }
    }
};
...
class OptimizeReservoirWithInflowsSDDP: public libstoch::OptimizerSDDPBase
{
...
private :
    ...
    std::shared_ptr<libstoch::OneDimData<libstoch::OneDimRegularSpaceGrid, double> > m_timeInflowAver ; /// store the average  inflow depending on time
    ...
    std::shared_ptr<libstoch::OneDimData<libstoch::OneDimRegularSpaceGrid, double> > m_timeDAverage; /// store the average demand depending on time
    ...
    std::shared_ptr<libstoch::OneDimData<libstoch::OneDimRegularSpaceGrid, double> > m_timeSpot; ///< store the grid of spot price depending on time
    ...
    void createAndSolveLP(const libstoch::SDDPCutOptBase   &p_linCut, const Eigen::ArrayXd &p_stateLevel,
                          ...
                          Eigen::ArrayXd &p_stateFollowing, double &p_cost) const
    {
        ...
        for (int isto = 0 ; isto < m_nbStorage; ++isto)
        {
            ...
            upperBound(isto + m_nbStorage) = libstoch::infty;
        }
        // quantity bought on the spot
        lowBound(2 * m_nbStorage) = 0.;
        upperBound(2 * m_nbStorage) = libstoch::infty;
        // for  fictitious data for bellman
        lowBound(2 * m_nbStorage + 1) = - libstoch::infty;
        upperBound(2 * m_nbStorage + 1) = libstoch::infty;
...
/// \param   p_timeSpot            spot price
    ...
    OptimizeReservoirWithInflowsSDDP(const double &p_initialLevel,
                                     ...
                                     const double &p_sigF,  const std::shared_ptr<libstoch::OneDimData<libstoch::OneDimRegularSpaceGrid, double> >    &p_timeInflowAver,
                                     const  double   &p_sigD,  const std::shared_ptr<libstoch::OneDimData<libstoch::OneDimRegularSpaceGrid, double> > &p_timeDAverage,
                                     const  std::shared_ptr<libstoch::OneDimData<libstoch::OneDimRegularSpaceGrid, double> >   &p_timeSpot,
    ...
    Eigen::ArrayXd oneStepBackward(const libstoch::SDDPCutOptBase &p_linCut, const std::tuple< std::shared_ptr<Eigen::ArrayXd>, int, int > &p_aState,
                                   const Eigen::ArrayXd &p_particle, const int &p_isample) const
    {
        ...
        double demand  ;
        if (libstoch::isLesserOrEqual(0., m_date))
        {
            for (int isto = 0; isto < m_nbStorage; ++isto)
    ...
    double  oneStepForward(const Eigen::ArrayXd &p_aParticle,  Eigen::ArrayXd &p_state, Eigen::ArrayXd &p_stateToStore,
                           const libstoch::SDDPCutOptBase &p_linCut,
                           const int &p_isimu) const
    {
        ...
        Eigen::ArrayXd inflows(m_nbStorage);
        double demand ;
        if (libstoch::isLesserOrEqual(m_date, 0.))
        {
    ...
    {
        m_date = p_date ;
        if (libstoch::isLesserOrEqual(0., p_date))
        {
    ...
    std::shared_ptr< libstoch::SimulatorSDDPBase > getSimulatorBackward() const
    {
        return m_simulatorBackward ;
    }

    /// \brief get the forward simulator back
    std::shared_ptr< libstoch::SimulatorSDDPBase > getSimulatorForward() const
    {
