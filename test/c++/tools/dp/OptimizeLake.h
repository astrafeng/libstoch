...
class OptimizeLake : public libstoch::OptimizerDPBase
{
private :
...
public :
    ...
    ///              .
    virtual std::pair< Eigen::ArrayXXd, Eigen::ArrayXXd>   stepOptimize(const   std::shared_ptr< libstoch::SpaceGrid> &p_grid,
            ...
            const std::vector < std::shared_ptr< Eigen::ArrayXXd > > &p_phiIn) const
    {
        ...
        for (int is = 0; is < inflows.size(); ++is)
        {
            ...
            {
                std::shared_ptr<libstoch::Interpolator>  interpolatorCurrentStock = p_grid->createInterpolator(stockWithInflows);
            ...
            {
                cashSameStock(is) = -libstoch::infty;
                condExpSameStock(is) = -libstoch::infty;
            }
        }
        // withdrawal
        ...
        for (int is = 0; is < inflows.size(); ++is)
        {
            ...
            if (libstoch::isStrictlyLesser(withdrawalMax(is), 0.))
            {
                cashWithdrawalStock(is) = -libstoch::infty;
                condExpWithdrawalStock(is)  = -libstoch::infty;
                gainWithdrawal(is) = -libstoch::infty;
            }
            else
            {
                Eigen::ArrayXd stockWithInflows(1);
                ...
                // Interpolator
                std::shared_ptr<libstoch::Interpolator> interpolatorWithdrawalStock = p_grid->createInterpolator(stockWithInflows);
