...
class OptimizeFictitiousSwing: public libstoch::OptimizerDPBase
{
private :
...
public :
            ...
            const std::vector < std::shared_ptr< Eigen::ArrayXXd > > &p_phiIn) const
    {
        ...
        for (int j = 0 ; j < nbArb; ++j)
        {
            ...
            {
                std::shared_ptr<libstoch::Interpolator>  interpolatorNextStock = p_grid->createInterpolator(nextStock);
    ...
    void stepSimulate(const std::shared_ptr< libstoch::SpaceGrid> &, const std::vector< libstoch::GridAndRegressedValue  > &p_continuation,
    ...
    virtual void stepSimulateControl(const std::shared_ptr< libstoch::SpaceGrid> &, const std::vector< libstoch::GridAndRegressedValue  > &,
