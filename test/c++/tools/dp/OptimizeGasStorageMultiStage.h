...
class OptimizeGasStorageMultiStage : public libstoch::OptimizerMultiStageDPBase
{
private :
...
public :
    ...
    Eigen::ArrayXXd stepOptimize(const   std::shared_ptr< libstoch::SpaceGrid> &p_grid, const Eigen::ArrayXd   &p_stock,
                                 const std::vector<std::shared_ptr<libstoch::ContinuationValue>> &p_condEsp,
                                 const std::vector < std::shared_ptr< Eigen::ArrayXXd > > &p_phiIn) const
    {
        ...
        if ((libstoch::isStrictlyLesser(injectionMax, 0.) && libstoch::isStrictlyLesser(withdrawalMax, 0.)) ||
                (libstoch::isStrictlyLesser(withdrawalMax, -injectionMax)))
        {
            // not an admissible point
            solution.setConstant(-libstoch::infty);
            return solution;
        }
        ...
        if (libstoch::isStrictlyLesser(0., injectionMax))
        {
        ...
        if (libstoch::isStrictlyLesser(0., withdrawalMax))
        {
    ...
    inline std::shared_ptr< libstoch::SimulatorMultiStageDPBase > getSimulator() const
    {
        return m_simulator ;
    }
