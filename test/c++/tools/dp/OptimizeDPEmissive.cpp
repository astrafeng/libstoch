...
std::pair< ArrayXXd, ArrayXXd> OptimizeDPEmissive::stepOptimize(const   std::shared_ptr< libstoch::SpaceGrid> &p_grid, const ArrayXd   &p_stock,
        ...
        const std::vector < std::shared_ptr< ArrayXXd > > &) const
{
    ...
    solutionAndControl.first = ArrayXXd::Constant(m_simulator->getNbSimul(), 2, -libstoch::infty);
    solutionAndControl.second =  ArrayXXd::Constant(m_simulator->getNbSimul(), 1, -libstoch::infty);
    // demand
    ...
    for (int is = 0 ; is < m_simulator->getNbSimul(); ++is)
    {
        for (int  iAl = 0; iAl < m_lMax / m_lStep ; ++iAl) // test all command for investment between 0 and lMax
        {
            ...
            if (p_grid->isInside(ptStockNext))
            {
                ...
                std::shared_ptr<libstoch::Interpolator>  interpolator = p_grid->createInterpolator(ptStockNext);
        ...
        if (libstoch::almostEqual(solutionAndControl.first(is, 0), - libstoch::infty, 10))
        {
...
void OptimizeDPEmissive::stepSimulate(const std::shared_ptr< libstoch::SpaceGrid>   &p_grid, const std::vector< libstoch::GridAndRegressedValue > &p_continuation,
