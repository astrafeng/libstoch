...
#include <Eigen/Dense>
#include <memory>
#include "libstoch/core/grids/LinearInterpolator.h"
#include "libstoch/core/grids/Interpolator.h"
...
#include "libstoch/core/grids/OneDimSpaceGrid.h"
...
class OptimizeGasStorageSwitchingCost : public libstoch::OptimizerDPBase
{
private :
    ...
    std::shared_ptr<libstoch::OneDimData<libstoch::OneDimSpaceGrid, int> > m_regime; ///< defines the number of regimes
...
public :
        ...
        m_switchCost(p_switchCost)
    {
        ...
        tval(0) = 0. ;
        tval(1) = libstoch::infty;
        std::shared_ptr<libstoch::OneDimSpaceGrid> timeGrid = std::make_shared<libstoch::OneDimSpaceGrid>(tval);
        std::shared_ptr<std::vector< int > > values =  std::make_shared<std::vector< int > >(2);
        (*values)[0] = 3;
        (*values)[1] = 3;
        m_regime = std::make_shared<libstoch::OneDimData<libstoch::OneDimSpaceGrid, int> >(timeGrid, values);
    }
    ...
    OptimizeGasStorageSwitchingCost(const double   &p_injectionRate, const double &p_withdrawalRate,
                                    const double &p_injectionCost, const double &p_withdrawalCost,
                                    const double &p_switchCost,
                                    const std::shared_ptr<libstoch::OneDimData<libstoch::OneDimSpaceGrid, int> > &p_regime):
        m_injectionRate(p_injectionRate), m_withdrawalRate(p_withdrawalRate), m_injectionCost(p_injectionCost), m_withdrawalCost(p_withdrawalCost),
        m_switchCost(p_switchCost),
        m_regime(p_regime)
    {}
    ...
    ///
    std::pair< Eigen::ArrayXXd, Eigen::ArrayXXd> stepOptimize(const std::shared_ptr< libstoch::SpaceGrid> &p_grid, const Eigen::ArrayXd   &p_stock,
            const std::vector<libstoch::ContinuationValue> &p_condEsp,
            const std::vector < std::shared_ptr< Eigen::ArrayXXd > > &p_phiIn) const
    {
        ...
        if (libstoch::isStrictlyLesser(injectionMax, 0.) && libstoch::isStrictlyLesser(withdrawalMax, 0.))
        {
            // not an admissible point
            solutionAndControl.first.setConstant(-libstoch::infty);
        ...
        if (p_grid->isInside(p_stock))
        {
            ...
            std::shared_ptr<libstoch::Interpolator>  interpolatorCurrentStock = p_grid->createInterpolator(p_stock);
        ...
        if ((libstoch::isStrictlyLesser(0., injectionMax)) && (nbRegReached >= 2))
        {
            Eigen::ArrayXd injectionStock = p_stock + injectionMax;
            // interpolator for stock level if injection
            std::shared_ptr<libstoch::Interpolator>  interpolatorInjectionStock = p_grid->createInterpolator(injectionStock);
        ...
        if ((libstoch::isStrictlyLesser(0., withdrawalMax)) && (nbRegReached == 3))
        {
            Eigen::ArrayXd withdrawalStock = p_stock - withdrawalMax;
            // interpolator for stock level if withdrawal
            std::shared_ptr<libstoch::Interpolator>   interpolatorWithdrawalStock = p_grid->createInterpolator(withdrawalStock);
            // cash flow  at previous step at injection level
        ...
        for (int iReg = 0; iReg < nbReg; ++ iReg)
        {
            ...
            else if (cashWithdrawalStock.size() > 0)
            {
                ...
                else
                {
                    ...
                    {
                        solutionAndControl.first.col(iReg).setConstant(-libstoch::infty);
...
/// \param p_phiInOut      defines the value function (modified): size number of functions to follow
    void stepSimulate(const std::shared_ptr< libstoch::SpaceGrid>   &p_grid, const std::vector< libstoch::GridAndRegressedValue  > &p_continuation,
                      libstoch::StateWithStocks &p_state,  Eigen::Ref<Eigen::ArrayXd> p_phiInOut) const
    {
        // actualization
        ...
        // if do nothing
        double espCondMax = - libstoch::infty;
    ...
    inline std::shared_ptr< libstoch::SimulatorDPBase > getSimulator() const
    {
        return m_simulator ;
    }
