...
class OptimizeGasStorageTree : public libstoch::OptimizerDPTreeBase
{
private :
...
public :
    ...
    std::pair< Eigen::ArrayXXd, Eigen::ArrayXXd> stepOptimize(const   std::shared_ptr< libstoch::SpaceGrid> &p_grid, const Eigen::ArrayXd   &p_stock,
            const std::vector<libstoch::ContinuationValueTree> &p_condEsp) const
    {
...
/// \param p_phiInOut      defines the value functions (modified): size number of functions to follow
    void stepSimulate(const std::shared_ptr< libstoch::SpaceGrid>   &p_grid, const std::vector< libstoch::GridTreeValue  > &p_continuation,
                      libstoch::StateTreeStocks &p_state, Eigen::Ref<Eigen::ArrayXd> p_phiInOut) const
    {
    ...
    inline std::shared_ptr< libstoch::SimulatorDPBaseTree > getSimulator() const
    {
        return m_simulator ;
    }
