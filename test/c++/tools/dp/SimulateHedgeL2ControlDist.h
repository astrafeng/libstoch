...
double SimulateHedgeL2ControlDist(const std::shared_ptr<libstoch::FullGrid> &p_grid,
                                  ...
                                  const boost::mpi::communicator &p_world)
{
    ...
    libstoch::StateWithStocks initState(0, Eigen::ArrayXd::Zero(simulator->getDimension()), initAssetValue);
    ...
    for (int istep = 0; istep < nbStep; ++istep)
    {
        ...
        libstoch::SimulateStepRegressionControlDist(ar, istep, nameAr, p_grid, p_grid, std::static_pointer_cast<libstoch::OptimizerDPBase>(p_optimize), true, p_world).oneStep(states, costFunction);
