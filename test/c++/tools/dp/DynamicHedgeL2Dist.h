...
double  DynamicHedgeL2Dist(const std::shared_ptr<libstoch::FullGrid> &p_grid,
                           ...
                           const boost::mpi::communicator &p_world)
{
    ...
    std::vector< std::shared_ptr< Eigen::ArrayXXd > >  valuesNext = libstoch::FinalStepDPDist(p_grid, p_optimize->getNbRegime(), p_optimize->getDimensionToSplit(), p_world)(p_funcFinalValue, assetPrev);
    ...
    for (int iStep = 0; iStep < nStepGlobal; ++iStep)
    {
        ...
        // transition object
        libstoch::TransitionStepRegressionDPDist transStep(p_grid, p_grid, std::static_pointer_cast<libstoch::OptimizerDPBase>(p_optimize), p_world);
    ...
    return libstoch::reconstructProc0Mpi(noFuture, p_grid, valuesNext[0], p_optimize->getDimensionToSplit(), p_world).mean();
}
