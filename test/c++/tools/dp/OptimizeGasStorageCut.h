...
#include "libstoch/regression/ContinuationCuts.h"
...
class OptimizeGasStorageCut : public libstoch::OptimizerDPCutBase, OptimizeGasStorageCutBase
{
private :
...
public :
    ...
    Eigen::ArrayXXd  stepOptimize(const   std::shared_ptr< libstoch::SpaceGrid> &p_grid, const Eigen::ArrayXd   &p_stock,
                                  const std::vector<libstoch::ContinuationCuts> &p_condEsp) const
    {
...
/// \param p_phiInOut      defines the value functions (modified): size number of functions to follow
    void stepSimulate(const std::shared_ptr< libstoch::SpaceGrid>   &p_grid, const std::vector< libstoch::ContinuationCuts  > &p_continuation,
