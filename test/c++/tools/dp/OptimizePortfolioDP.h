...
class OptimizePortfolioDP : public libstoch::OptimizerNoRegressionDPBase
{
private :
...
public :
    ...
    virtual std::pair< Eigen::ArrayXXd, Eigen::ArrayXXd>   stepOptimize(const Eigen::ArrayXd   &p_stock,
            const std::vector< libstoch::GridAndRegressedValue>  &p_valNext,
            std::shared_ptr< libstoch::BaseRegression  >     p_regressorCur) const
    {
        // convert
        std::shared_ptr< libstoch::LocalConstRegression> constRegressor  = std::static_pointer_cast<libstoch::LocalConstRegression>(p_regressorCur);
        ...
        std::shared_ptr< libstoch::SpaceGrid >  grid = p_valNext[0].getGrid();
        // level min, max of the portfolio considered
        double minA = grid->getExtremeValues()[0][0];
        double maxA = grid->getExtremeValues()[0][1];
        // is current time 0 ?
        bool bZeroDate = libstoch::isLesserOrEqual(m_simulator-> getCurrentStep(), 0.);
        ...
        Eigen::ArrayXd optCommand = Eigen::ArrayXd::Constant(nbCell, -libstoch::infty);
        // cash expectation
        Eigen::ArrayXd espCash(nbCell);
        Eigen::ArrayXd espCashMax = Eigen::ArrayXd::Constant(nbCell, -libstoch::infty);
    ...
    virtual void stepSimulateControl(const std::shared_ptr< libstoch::SpaceGrid> &, const std::vector< libstoch::GridAndRegressedValue  > &p_control,
