...
#include "libstoch/regression/LocalRegression.h"
#include "libstoch/regression/globalL2HedgeMinimize.h"

/* \file OptimizeOptionL2.h
 * \brief Permits to optimize and simulate in one step at one stock level (in hedging product)  in libstoch  framework
 *        an option with a global variance minimization.
 * \author Xavier Warin
 */
template< class Simulator>
class OptimizeOptionL2: public libstoch::OptimizerDPBase //, public libstoch::OptimizerNoRegressionDPBase
{
private :
...
public :
    ...
    std::vector< std::array< double, 2> > getCone(const  std::vector< std::array< double, 2>  > &) const
    {
        ...
        {
            extrGrid[i][0] =  -libstoch::infty; // get all Bellmna Values independently  of the position
            extrGrid[i][1] =   libstoch::infty; // get all Beelman values independently  of the position
        }
        return extrGrid;
    }
    ...
    ///
    std::pair< Eigen::ArrayXXd, Eigen::ArrayXXd> stepOptimize(const std::shared_ptr< libstoch::SpaceGrid> &p_grid,
            ...
            const std::vector < std::shared_ptr< Eigen::ArrayXXd > > &p_phiIn) const
    {
        ...
        for (int id = 0; id < posMin.size(); ++id)
        {
            ...
            nbCommand(id) = static_cast<int>((posMax - posMin(id) + libstoch::tiny) / m_stepForHedge(id));
            stepCommand(id) = (posMax - posMin(id)) / nbCommand(id);
        }
        // command grid
        std::shared_ptr<libstoch::SpaceGrid>  commands = std::make_shared<libstoch::RegularSpaceGrid>(posMin, stepCommand, nbCommand);
        std::pair<Eigen::ArrayXd, Eigen::ArrayXXd> valueAndHedge;
        std::shared_ptr< libstoch::BaseRegression >  regressor = p_condEsp[0].getCondExp();
    ...
    void stepSimulateControl(const std::shared_ptr< libstoch::SpaceGrid>    &p_grid,
                             const std::vector< libstoch::GridAndRegressedValue  > &p_control,
    ...
    void stepSimulate(const std::shared_ptr< libstoch::SpaceGrid> &,
                      const std::vector< libstoch::GridAndRegressedValue  > &,
                      libstoch::StateWithStocks &,
