...
#include "libstoch/dp/SimulateStepRegression.h"
...
double simuDPNonEmissive(const shared_ptr<libstoch::SpaceGrid> &p_grid,
                         ...
                         const boost::mpi::communicator &p_world)
{
    ...
    vector< libstoch::StateWithStocks> states;
    states.reserve(simulator->getNbSimul());
    ArrayXXd  iniStoState = simulator->getParticles().array();
    for (int is = 0; is < simulator->getNbSimul(); ++is)
        states.push_back(libstoch::StateWithStocks(0, p_pointStock, iniStoState.col(is)));
    ...
    for (int istep = 0; istep < nbStep; ++istep)
    {
        ...
        libstoch::SimulateStepRegression(ar, nbStep - 1 - istep, nameAr, p_grid, p_optimize, p_world).oneStep(states, costFunction);
