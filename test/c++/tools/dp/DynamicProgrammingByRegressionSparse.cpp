...
#include "libstoch/dp/TransitionStepRegressionDPSparse.h"
#include "libstoch/dp/OptimizerDPBase.h"

using namespace std;


double  DynamicProgrammingByRegressionSparse(const shared_ptr<libstoch::SparseSpaceGrid> &p_grid,
                                            ...
                                            )
{
    ...
    // final values
    std::vector< shared_ptr< Eigen::ArrayXXd > >  valuesNext = libstoch::FinalStepDP(p_grid, p_optimize->getNbRegime())(p_funcFinalValue, simulator->getParticles().array());
    ...
    for (int iStep = 0; iStep < simulator->getNbStep(); ++iStep)
    {
        ...
        // transition object
        libstoch::TransitionStepRegressionDPSparse transStep(p_grid, p_grid, p_optimize
#ifdef USE_MPI
                , p_world
#endif
                                                         );
