...
#include "libstoch/dp/TransitionStepDPDist.h"
...
double  DynamicProgrammingPortfolioDist(const shared_ptr<libstoch::FullGrid> &p_grid,
                                       ...
                                       )
{
    ...
    {
        vector< shared_ptr< ArrayXXd > >  valuesPrevious = libstoch::FinalStepDPDist(p_grid, p_optimize->getNbRegime(), p_optimize->getDimensionToSplit(), p_world)(p_funcFinalValue, *p_optimize->getCurrentSim());
        // regressor operator
    ...
    for (int iStep = 0; iStep < p_optimize->getNbStep(); ++iStep)
    {
        ...
        // transition object
        libstoch::TransitionStepDPDist transStep(p_grid, p_grid, regressorCur, regressorPrevious, p_optimize, p_world);
    ...
    return libstoch::reconstructProc0Mpi(p_initialPortfolio, p_grid, topRows, p_optimize->getDimensionToSplit(), p_world).mean();
}
#endif
