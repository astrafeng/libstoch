...
#include "libstoch/dp/TransitionStepRegressionDPCut.h"
...
double  DynamicProgrammingByRegressionCut(const shared_ptr<libstoch::FullGrid> &p_grid,
                                         ...
                                         )
{
    ...
    // final cut values
    vector< shared_ptr< ArrayXXd > >  valueCutsNext = libstoch::FinalStepDPCut(p_grid, p_optimize->getNbRegime())(p_funcFinalValue, simulator->getParticles().array());
    ...
    for (int iStep = 0; iStep < simulator->getNbStep(); ++iStep)
    {
        ...
        // transition object
        libstoch::TransitionStepRegressionDPCut  transStep(p_grid, p_grid, p_optimize
#ifdef USE_MPI
                , p_world
#endif
                                                       );
