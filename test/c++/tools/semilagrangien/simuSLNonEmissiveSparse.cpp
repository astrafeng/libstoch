...
#include "libstoch/semilagrangien/SimulateStepSemilagrang.h"

using namespace std;
using namespace Eigen ;

double simuSLNonEmissiveSparse(const shared_ptr<libstoch::SparseSpaceGrid> &p_grid,
                               ...
                               const boost::mpi::communicator &p_world)
{
    ...
    for (int istep = 0; istep < p_nbStep; ++istep)
    {
        ...
        libstoch::SimulateStepSemilagrang(ar, p_nbStep - 1 - istep, nameAr, p_grid, p_optimize, p_world).oneStep(gaussian, states, regime, costFunction);
        if (p_world.rank() == 0)
        {
            *fileInvest << istep << " "  ;
            *fileQ << istep << " ";
