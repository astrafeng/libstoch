...
public :
    ...
    void stepSimulate(const SpaceGrid   &p_gridNext,
                      const  std::vector< std::shared_ptr< libstoch::SemiLagrangEspCond> > &p_semiLag,
    ...
    {
        if ((libstoch::almostEqual(p_point(0), m_extrem[0][0], 10)) || (libstoch::almostEqual(p_point(0), m_extrem[0][1], 10))
                || (libstoch::almostEqual(p_point(1), m_extrem[1][1], 10)))
            return false;
        return true;
    }
};
