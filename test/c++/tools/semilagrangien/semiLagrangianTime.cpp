...
pair< double, double>  semiLagrangianTime(const shared_ptr<libstoch::SpaceGrid> &p_grid,
...
{
    // final values
    vector< shared_ptr< ArrayXd > >  valuesNext = libstoch::InitialValue(p_grid, p_optimize->getNbRegime())(p_funcInitialValue);
    ...
    for (int iStep = 0; iStep < p_nStep ; ++iStep)
    {
        ...
        // transition object
        libstoch::TransitionStepSemilagrang transStep(p_grid, p_grid, p_optimize
#ifdef USE_MPI
                , p_world
#endif
                                                  );
    ...
    shared_ptr<libstoch::InterpolatorSpectral> gridInterpol =  p_grid->createInterpolatorSpectral(*valuesNext[p_initialRegime]);
