...
double simuSLNonEmissive(const shared_ptr<libstoch::FullGrid> &p_grid,
                         ...
                         const boost::mpi::communicator &p_world)
{
    ...
    for (int istep = 0; istep < p_nbStep; ++istep)
    {
        ...
        libstoch::SimulateStepSemilagrangDist(ar, p_nbStep - 1 - istep, nameAr, p_grid, p_optimize, true, p_world).oneStep(gaussian, states, regime, costFunction);
        if (p_world.rank() == 0)
        {
            *fileInvest << istep + 1 << " "  ;
            *fileQ << istep + 1 << " ";
