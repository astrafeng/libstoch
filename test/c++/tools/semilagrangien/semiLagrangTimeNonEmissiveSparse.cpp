...
#include "libstoch/semilagrangien/InitialValue.h"
#include "libstoch/semilagrangien/TransitionStepSemilagrang.h"
...
void  semiLagrangTimeNonEmissiveSparse(const shared_ptr<libstoch::SparseSpaceGrid> &p_grid,
                                       const shared_ptr<libstoch::OptimizeSLEmissive > &p_optimize,
...
{
    // final values
    vector< shared_ptr< ArrayXd > >  valuesNext = libstoch::InitialValue(dynamic_pointer_cast<libstoch::SpaceGrid>(p_grid), p_optimize->getNbRegime())(p_funcInitialValue);
    ...
    for (int iStep = 0; iStep < p_nStep ; ++iStep)
    {
        ...
        // transition object
#ifdef USE_MPI
        libstoch::TransitionStepSemilagrang transStep(p_grid, p_grid, static_pointer_cast<libstoch::OptimizerSLBase>(p_optimize), p_world);
#else
        libstoch::TransitionStepSemilagrang transStep(p_grid, p_grid, static_pointer_cast<libstoch::OptimizerSLBase>(p_optimize));
#endif
    ...
    if (p_world.rank() == 0)
    {
        ...
        shared_ptr<libstoch::InterpolatorSpectral> gridInterpol1 =  p_grid->createInterpolatorSpectral(vFunc1);
        shared_ptr<libstoch::InterpolatorSpectral> gridInterpol2 =  p_grid->createInterpolatorSpectral(vFunc2);
        shared_ptr<libstoch::InterpolatorSpectral> gridInterpControl = p_grid->createInterpolatorSpectral(vControl);
        // DUMP
