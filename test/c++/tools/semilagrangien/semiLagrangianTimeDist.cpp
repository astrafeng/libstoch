...
std::pair< double, double>  semiLagrangianTimeDist(const shared_ptr<libstoch::FullGrid> &p_grid,
        const shared_ptr<libstoch::OptimizerSLBase > &p_optimize,
        ...
        const boost::mpi::communicator &p_world)
{
    ...
    for (int iStep = 0; iStep < p_nStep ; ++iStep)
    {
        ...
        // transition object
        libstoch::TransitionStepSemilagrangDist transStep(p_grid, p_grid, p_optimize, p_world);
    ...
    if (p_world.rank() == 0)
    {
        ...
        shared_ptr<libstoch::InterpolatorSpectral> gridInterpol =  p_grid->createInterpolatorSpectral(*valuesRecons[p_initialRegime]);
        shared_ptr< libstoch::GridIterator> iterGrid = p_grid->getGridIterator();
        double errMax = 0. ;
