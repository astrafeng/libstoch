...
#include "libstoch/semilagrangien/InitialValueDist.h"
#include "libstoch/semilagrangien/TransitionStepSemilagrangDist.h"
...
void  semiLagrangTimeNonEmissive(const shared_ptr<libstoch::FullGrid> &p_grid,
...
{
    // final values
    vector< shared_ptr< ArrayXd > >  valuesNext = libstoch::InitialValueDist(p_grid, p_optimize->getNbRegime(), p_optimize->getDimensionToSplit(), p_world)(p_funcInitialValue);
    // dump
    ...
    for (int iStep = 0; iStep < p_nStep ; ++iStep)
    {
        ...
        // transition object
        libstoch::TransitionStepSemilagrangDist transStep(p_grid, p_grid, static_pointer_cast<libstoch::OptimizerSLBase>(p_optimize), p_world);
    ...
    if (p_world.rank() == 0)
    {
        ...
        shared_ptr<libstoch::InterpolatorSpectral> gridInterpol1 =  p_grid->createInterpolatorSpectral(*valuesRecons[0]);
        shared_ptr<libstoch::InterpolatorSpectral> gridInterpol2 =  p_grid->createInterpolatorSpectral(*valuesRecons[1]);
        shared_ptr<libstoch::InterpolatorSpectral> gridInterpControl = p_grid->createInterpolatorSpectral(*optimalControl[0]);
        // DUMP
