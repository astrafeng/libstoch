...
class MeanRevertingSimulator: public libstoch::SimulatorMultiStageDPBase, public libstoch::SimulatorSDDPBase
{
protected :
    ...
    {
        if (libstoch:: isLesserOrEqual(m_currentStep, libstoch::zero))
        {
            m_OUProcess.setConstant(libstoch::zero);
        }
        else
        {
            for (int id = 0; id < m_OUProcess.rows(); ++id)
