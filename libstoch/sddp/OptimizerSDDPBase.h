...
#include "libstoch/sddp/SimulatorSDDPBase.h"
#include "libstoch/sddp/SimulatorSDDPBaseTree.h"
...
public :
    ...
    virtual Eigen::ArrayXd oneStepBackward(const libstoch::SDDPCutOptBase &p_linCut, const std::tuple< std::shared_ptr<Eigen::ArrayXd>, int, int > &p_aState, const Eigen::ArrayXd &p_particle, const int &p_isample) const = 0;
    ...
    virtual double oneStepForward(const Eigen::ArrayXd &p_aParticle, Eigen::ArrayXd &p_state,  Eigen::ArrayXd &p_stateToStore, const libstoch::SDDPCutOptBase &p_linCut,
    ...
    virtual std::shared_ptr< libstoch::SimulatorSDDPBase > getSimulatorBackward() const = 0;

    /// \brief get the forward simulator back
    virtual std::shared_ptr< libstoch::SimulatorSDDPBase > getSimulatorForward() const = 0;
