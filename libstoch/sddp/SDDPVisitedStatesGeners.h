...
#include "libstoch/sddp/SDDPVisitedStates.h"
...
///@{
gs_specialize_class_id(libstoch::SDDPVisitedStates, 1)
gs_declare_type_external(libstoch::SDDPVisitedStates)
///@}
namespace gs
{
//
...
struct GenericWriter < Stream, State, libstoch::SDDPVisitedStates,   Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool process(const libstoch::SDDPVisitedStates &p_state, Stream &p_os,
                               State *, const bool p_processClassId)
    {
        // If necessary, serialize the class id
        static const ClassId current(ClassId::makeId<libstoch::SDDPVisitedStates>());
...
//
template <class Stream, class State>
struct GenericReader < Stream, State, libstoch::SDDPVisitedStates,   Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool readIntoPtr(libstoch::SDDPVisitedStates *&ptr, Stream &p_is,
                                   State *p_st, const bool p_processClassId)
    {
        ...
        if (ptr)
            *ptr = libstoch::SDDPVisitedStates(*meshToState, *states, *mesh);
        else
            ptr = new  libstoch::SDDPVisitedStates(*meshToState, *states, *mesh);
        return true;
    }

    inline static bool process(libstoch::SDDPVisitedStates &s, Stream &is,
                               State *st, const bool p_processClassId)
    {
        // Simply convert reading by reference into reading by pointer
        libstoch::SDDPVisitedStates *ps = &s;
