...
#include <memory>
#include "libstoch/sddp/LocalConstRegressionForSDDP.h"
...
///@{
gs_specialize_class_id(libstoch::LocalConstRegressionForSDDP, 1)
/// an external class
gs_declare_type_external(libstoch::LocalConstRegressionForSDDP)
///@}
...
struct GenericWriter < Stream, State, libstoch::LocalConstRegressionForSDDP,
           Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool process(const libstoch::LocalConstRegressionForSDDP &p_regression, Stream &p_os,
                               State *, const bool p_processClassId)
    {
        // If necessary, serialize the class id
        static const ClassId current(ClassId::makeId<libstoch::LocalConstRegressionForSDDP>());
...
//
template <class Stream, class State>
struct GenericReader < Stream, State, libstoch::LocalConstRegressionForSDDP,
           Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool readIntoPtr(libstoch::LocalConstRegressionForSDDP  *&ptr, Stream &p_is,
                                   State *p_st, const bool p_processClassId)
    {
        ...
        if (ptr)
            *ptr = libstoch::LocalConstRegressionForSDDP(bZeroDate, *particles, *nbMesh, *mesh, *mesh1D, *simToCell, *matReg, *simulBelongingToCell, *meanX, *etypX, *svdMatrix, bRotationAndRescale);
        else
        {
            ptr = new  libstoch::LocalConstRegressionForSDDP(bZeroDate, *particles, *nbMesh, *mesh, *mesh1D, *simToCell, *matReg,  *simulBelongingToCell, *meanX, *etypX, *svdMatrix, bRotationAndRescale);
        }
        return true;
    }

    inline static bool process(libstoch::LocalConstRegressionForSDDP &s, Stream &is,
                               State *st, const bool p_processClassId)
    {
        // Simply convert reading by reference into reading by pointer
        libstoch::LocalConstRegressionForSDDP *ps = &s;
