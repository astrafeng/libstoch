...
#include "libstoch/sddp/SDDPVisitedStatesTree.h"
...
///@{
gs_specialize_class_id(libstoch::SDDPVisitedStatesTree, 1)
gs_declare_type_external(libstoch::SDDPVisitedStatesTree)
///@}
namespace gs
{
//
...
struct GenericWriter < Stream, State, libstoch::SDDPVisitedStatesTree,   Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool process(const libstoch::SDDPVisitedStatesTree &p_state, Stream &p_os,
                               State *, const bool p_processClassId)
    {
        // If necessary, serialize the class id
        static const ClassId current(ClassId::makeId<libstoch::SDDPVisitedStatesTree>());
...
//
template <class Stream, class State>
struct GenericReader < Stream, State, libstoch::SDDPVisitedStatesTree,   Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool readIntoPtr(libstoch::SDDPVisitedStatesTree *&ptr, Stream &p_is,
                                   State *p_st, const bool p_processClassId)
    {
        ...
        if (ptr)
            *ptr = libstoch::SDDPVisitedStatesTree(*meshToState, *states, *mesh);
        else
            ptr = new  libstoch::SDDPVisitedStatesTree(*meshToState, *states, *mesh);
        return true;
    }

    inline static bool process(libstoch::SDDPVisitedStatesTree &s, Stream &is,
                               State *st, const bool p_processClassId)
    {
        // Simply convert reading by reference into reading by pointer
        libstoch::SDDPVisitedStatesTree *ps = &s;
