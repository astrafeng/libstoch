...
#include <memory>
#include "libstoch/sddp/LocalLinearRegressionForSDDP.h"
...
///@{
gs_specialize_class_id(libstoch::LocalLinearRegressionForSDDP, 1)
/// an external class
gs_declare_type_external(libstoch::LocalLinearRegressionForSDDP)
///@}
...
struct GenericWriter < Stream, State, libstoch::LocalLinearRegressionForSDDP,
           Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool process(const libstoch::LocalLinearRegressionForSDDP &p_regression, Stream &p_os,
                               State *, const bool p_processClassId)
    {
        // If necessary, serialize the class id
        static const ClassId current(ClassId::makeId<libstoch::LocalLinearRegressionForSDDP>());
...
//
template <class Stream, class State>
struct GenericReader < Stream, State, libstoch::LocalLinearRegressionForSDDP,
           Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool readIntoPtr(libstoch::LocalLinearRegressionForSDDP  *&ptr, Stream &p_is,
                                   State *p_st, const bool p_processClassId)
    {
        ...
        if (ptr)
        {
            *ptr = libstoch::LocalLinearRegressionForSDDP(bZeroDate, *particles, *nbMesh, *mesh, *mesh1D, *simToCell, *matReg, *diagReg, *simulBelongingToCell, *meanX, *etypX, *svdMatrix, bRotationAndRescale);
        }
        else
        {
            ptr = new  libstoch::LocalLinearRegressionForSDDP(bZeroDate, *particles, *nbMesh, *mesh, *mesh1D, *simToCell, *matReg, *diagReg, *simulBelongingToCell, *meanX, *etypX, *svdMatrix, bRotationAndRescale);
        }

        return true;
    }

    inline static bool process(libstoch::LocalLinearRegressionForSDDP &s, Stream &is,
                               State *st, const bool p_processClassId)
    {
        // Simply convert reading by reference into reading by pointer
        libstoch::LocalLinearRegressionForSDDP *ps = &s;
